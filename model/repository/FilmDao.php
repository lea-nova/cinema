...
class FilmDao extends Dao
{
    ...
    public static function getOne(int $id): Film
    {
        ...
        $data = $query->fetch();
        $roles = new Role($data['id'], $data['personnage'], new Acteur($data['id'], $data['nom'], $data['prenom']));
        return new Film($data['id'], $data['titre'], $data['realisateur'], $data['affiche'], $data['annee'], $data['role']);
    }

    public static function search($query)
    {
        $query = self::$bdd->prepare('SELECT * FROM film WHERE titre LIKE :query');
        $searchQuery = '%' . $query . '%';
        $query->bindParam(':query', $searchQuery);
        $query->execute();
        $results = [];
        while ($data = $query->fetch()) {
            $results[] = new Film($data['id'], $data['titre'], $data['realisateur'], $data['affiche'], $data['annee'], $data['roles']);
        }
        return $results;
    }
}
